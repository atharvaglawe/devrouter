 ...
 * Analyze Command
 *
 * Indexes a repository and stores the knowledge graph in .codegraph/
 *
...
export const analyzeCommand = async (inputPath?: string, options?: AnalyzeOptions) => {
  ...
  if (options?.verbose) {
    process.env.CODEGRAPH_VERBOSE = '1';
    process.env.GITNEXUS_VERBOSE = '1'; // legacy fallback for code that hasn't migrated yet
  }
  ...
  if (process.env.CODEGRAPH_NO_GITIGNORE || process.env.GITNEXUS_NO_GITIGNORE) {
    console.log(
      '  CODEGRAPH_NO_GITIGNORE is set — skipping .gitignore (still reading .codegraphignore / .gitnexusignore)\n',
    );
  }
  ...
  try {
    ...
    // Note: upstream gitnexus prints a `gitnexus setup` tip here for first-time
    // users. The slim devrouter fork has no MCP server, so the tip was removed.
