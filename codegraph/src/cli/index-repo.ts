 ...
 * Index Command
 *
 * Registers an existing .codegraph/ (or legacy .gitnexus/) folder into the
 * global registry so the HTTP server can discover the repo without running
 * a full `codegraph analyze`.
 *
 * Useful when a pre-built index directory is already present (e.g. after
...
export const indexCommand = async (inputPathParts?: string[], options?: IndexOptions) => {
  ...
  if (!options?.allowNonGit && !isGitRepo(repoPath)) {
    ...
    console.log('  Or use --allow-non-git to register an existing .codegraph index anyway.\n');
    process.exitCode = 1;
    return;
  }
  ...
  try {
    ...
    console.log(`  No .codegraph/ (or .gitnexus/) folder found at: ${storagePath}`);
    console.log('  Run `codegraph analyze` to build the index first.\n');
    process.exitCode = 1;
    return;
  }
  ...
  try {
    ...
    console.log(`  Storage folder exists but contains no LadybugDB index.`);
    console.log('  Run `codegraph analyze` to build the index.\n');
    process.exitCode = 1;
    return;
  }
  ...
  if (!meta) {
    if (!options?.force) {
      console.log(`  Storage folder exists but meta.json is missing.`);
      console.log('  Use --force to register anyway (stats will be empty),');
      console.log('  or run `codegraph analyze` to rebuild properly.\n');
      process.exitCode = 1;
      return;
    }
