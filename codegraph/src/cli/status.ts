...
export const statusCommand = async () => {
  ...
  if (!repo) {
    ...
    if (await hasKuzuIndex(storagePath)) {
      ...
      console.log('Run: codegraph analyze   (rebuilds the index with LadybugDB)');
    } else {
      console.log('Repository not indexed.');
      console.log('Run: codegraph analyze   (or `devrouter analyze`)');
    }
    return;
  }
  ...
  console.log(`Status: ${isUpToDate ? '✅ up-to-date' : '⚠️ stale (re-run codegraph analyze)'}`);
};
