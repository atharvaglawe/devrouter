import type { GraphNode, GraphRelationship, NodeLabel } from '../../_shared/index.js';
...
const processParsingWithWorkers = async (
  ...
  const allSwitchCases: ExtractedSwitchCase[] = [];
  ...
  for (const result of chunkResults) {
    ...
    if (result.switchCases) for (const _item of result.switchCases) allSwitchCases.push(_item);
