...
/**
 * Resolve pre-extracted switch-case constant references to ACCESSES edges.
 * Handles both qualified (pkg.Const) and bare (Const) forms.
 */
export const processSwitchCasesFromExtracted = (
  graph: KnowledgeGraph,
  switchCases: ExtractedSwitchCase[],
  ctx: ResolutionContext,
): void => {
  for (const sc of switchCases) {
    // Qualified form: receiverName.constantName (e.g., shortnames.ZeroClickAffinityProvider)
    // The receiver is an import alias — resolve the constant as a symbol in the imported package.
    if (sc.receiverName) {
      const resolved = ctx.resolve(sc.constantName, sc.filePath);
      if (resolved && resolved.candidates.length > 0) {
        const target = resolved.candidates[0];
        graph.addRelationship({
          id: generateId('ACCESSES', `${sc.sourceId}:${target.nodeId}:switch-case`),
    ...
    // Bare form or qualified fallback: resolve as a local/imported symbol
    const resolved = ctx.resolve(sc.constantName, sc.filePath);
    if (resolved && resolved.candidates.length > 0) {
      const target = resolved.candidates[0];
      graph.addRelationship({
