...
export interface ExtractedSwitchCase {
  filePath: string;
  /** generateId of enclosing function, or generateId('File', filePath) for top-level */
  sourceId: string;
  /** Constant name referenced in the case expression (e.g., 'ZeroClickAffinityProvider') */
  constantName: string;
  /** Package/receiver qualifier if present (e.g., 'shortnames' from shortnames.ZeroClickAffinityProvider) */
  receiverName?: string;
}
...
export interface ParseWorkerResult {
  ...
  switchCases: ExtractedSwitchCase[];
...
const processFileGroup = (
  ...
  for (const file of files) {
    ...
    for (const match of matches) {
      ...
      // Extract switch-case constant references
      if (captureMap['switch_case'] && captureMap['switch_case.name']) {
        const constantName = captureMap['switch_case.name'].text;
        if (constantName) {
          const srcId =
            findEnclosingFunctionId(captureMap['switch_case'], file.path, provider) ||
            generateId('File', file.path);
          const receiverName = captureMap['switch_case.receiver']?.text;
          result.switchCases.push({
            filePath: file.path,
            sourceId: srcId,
            constantName,
...
const mergeResult = (target: ParseWorkerResult, src: ParseWorkerResult) => {
  ...
  appendAll(target.switchCases, src.switchCases);
