...
async function runChunkedParseAndResolve(
  ...
  const deferredSwitchCases: ExtractedSwitchCase[] = [];
  ...
  try {
    for (let chunkIdx = 0; chunkIdx < numChunks; chunkIdx++) {
      ...
      if (chunkWorkerData) {
        ...
        if (chunkWorkerData.switchCases?.length) {
          for (const _item of chunkWorkerData.switchCases) deferredSwitchCases.push(_item);
        }
    ...
    if (deferredSwitchCases.length > 0) {
      processSwitchCasesFromExtracted(graph, deferredSwitchCases, ctx);
    }
  } finally {
