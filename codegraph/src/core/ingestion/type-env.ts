...
const resolveFixpointBindings = (
  ...
  for (let iter = 0; iter < MAX_FIXPOINT_ITERATIONS; iter++) {
    ...
    if (!changed) break;
    if (iter === MAX_FIXPOINT_ITERATIONS - 1 && (process.env.CODEGRAPH_DEBUG || process.env.GITNEXUS_DEBUG)) {
...
export const buildTypeEnv = (
  ...
  // Iterative DFS — recursive walks overflow the stack on pathologically deep ASTs
  // (e.g. long chains of nested blocks/calls in generated Go/JS).
  type WalkFrame = { node: SyntaxNode; currentScope: string };
  const walkStack: WalkFrame[] = [{ node: tree.rootNode, currentScope: FILE_SCOPE }];

  while (walkStack.length > 0) {
    const frame = walkStack.pop()!;
    const { node, currentScope } = frame;
    ...
    // Children: push last child first so we match recursive pre-order (child 0 first).
    for (let i = node.childCount - 1; i >= 0; i--) {
      const child = node.child(i);
      if (child) walkStack.push({ node: child, currentScope: scope });
    }
  }
