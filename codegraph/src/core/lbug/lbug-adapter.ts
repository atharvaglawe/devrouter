 ...
 * an exclusive lock on the LadybugDB file (e.g. `codegraph analyze` or
 * `codegraph serve` running at the same time).
 */
 ...
 * database is busy (e.g. `codegraph analyze` holds the write lock).
...
const doInitLbug = async (dbPath: string) => {
  ...
  // Load FTS + VECTOR extensions on the singleton connection.
  //
  // The HTTP server (api.ts) routes every /api/search and /api/query through
  // `withLbugDb` → this connection. If FTS isn't loaded here, every BM25 /
  // hybrid search returns empty silently — `bm25-index.ts`'s no-repoId path
  // wraps each FTS call in `.catch(() => [])`, which used to mask this for
  // every repo with no error in any log.
  //
  // Both calls are idempotent (extensions track loaded state on `ftsLoaded`
  // / `vectorExtensionLoaded`) and both throw on hard failures (extension
  // binary missing, etc.) so problems become visible at server start time
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        // Throw so callers (e.g. createFTSIndex during analyze) see the real
        // cause instead of a downstream "FTS function not found" error.
        // Logging-only here used to swallow indexer failures silently and
        // leave BM25 search permanently broken with no visible error.
        console.error('codegraph: FTS extension load failed:', msg);
        throw new Error(`FTS extension load failed: ${msg}`);
      }
    }
  }
};
...
export const loadVectorExtension = async (): Promise<void> => {
  ...
  try {
    ...
    if (
      ...
      // Hard fail — earlier this branch only console.error'd and fell
      // through with vectorExtensionLoaded still false. That meant every
      // downstream CREATE_VECTOR_INDEX / QUERY_VECTOR_INDEX call ran
      // against a session with no vector extension loaded and silently
      // no-op'd, which is exactly the mode that wedged hybrid/semantic
      // search on mall for an entire bench day before we caught it.
      // Surfacing the error here gives callers (analyzer + HTTP server
      // startup) a chance to refuse to start rather than ship a broken
      // index.
      console.error('codegraph: VECTOR extension load failed:', msg);
