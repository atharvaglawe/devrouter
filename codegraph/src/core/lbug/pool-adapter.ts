...
/**
 * Load FTS + VECTOR extensions on every connection in `connections`.
 *
 * Idempotent per `shared` flag: if `shared.ftsLoaded` / `shared.vectorLoaded`
 * is already true we skip the work (this is called from both `doInitLbug`
 * and `initLbugWithDb`).
 *
 * Why not "load once": LadybugDB tracks loaded extensions per-Connection,
 * not per-Database. Loading on `available[0]` only used to leave the other
 * pool connections without the extension; queries routed to those
 * connections then failed at `CALL QUERY_FTS_INDEX(...)` and the failure
 * was swallowed by `bm25-index.ts`'s `catch { return [] }`. Net result was
 * search returning empty for every repo with no error in any log.
 *
 * Logs failures explicitly. The previous bare `catch {}` here turned a
 * fixable extension-install problem into a silent feature outage.
 */
async function loadExtensionsOnPool(
  repoId: string,
  connections: lbug.Connection[],
  shared: SharedDB,
): Promise<void> {
  if (!shared.ftsLoaded) {
    let ftsErr: Error | null = null;
    for (let i = 0; i < connections.length; i++) {
      try {
        await connections[i].query('LOAD EXTENSION fts');
      } catch (loadErr: any) {
        // Fall back to install + load — needed on a fresh DB where the
        // extension binary is present but never registered.
        try {
          await connections[i].query('INSTALL fts');
          await connections[i].query('LOAD EXTENSION fts');
        } catch (installErr: any) {
          const msg = installErr?.message || loadErr?.message || 'unknown';
          if (msg.includes('already loaded') || msg.includes('already installed')) {
            continue; // idempotent success on this conn, keep going
          }
          ftsErr = installErr instanceof Error ? installErr : new Error(String(installErr));
          break;
        }
      }
    }
    if (ftsErr) {
      console.error(
        `codegraph[${repoId}]: FTS extension failed to load on pool — ` +
          `BM25 search will return empty. Cause: ${ftsErr.message}`,
      );
    } else {
  ...
  if (!shared.vectorLoaded) {
    // Skip VECTOR extension when this repo has no embeddings on disk.
    //
    // LadybugDB's `LOAD EXTENSION VECTOR` walks the catalog and instantiates
    // an `OnDiskHNSWIndex` for every registered HNSW index. Older codegraph
    // versions called `CREATE_VECTOR_INDEX` during schema setup, before any
    // rows existed. Repos indexed by that path now have a catalog entry
    // pointing at zero on-disk pages — the constructor dereferences a null
    // pointer at offset 0x8 and segfaults the entire Node process. Crash
    // signature: SIGSEGV in lbug::vector_extension::OnDiskHNSWIndex from
    // ExtensionManager::loadExtension.
    //
    // Probing CodeEmbedding row count is cheap, never touches HNSW pages,
    // and is enough to gate the unsafe path: a repo with zero embeddings
    // cannot benefit from the vector extension anyway, so skipping it is
    // both safe and semantics-preserving.
    const hasEmbeddings = await probeHasEmbeddings(connections[0]);
    if (!hasEmbeddings) {
      shared.vectorLoaded = true; // mark resolved; do not re-probe on next conn
      return;
    }

    let vecErr: Error | null = null;
    for (let i = 0; i < connections.length; i++) {
      try {
        await connections[i].query('INSTALL VECTOR');
        await connections[i].query('LOAD EXTENSION VECTOR');
      } catch (err: any) {
        const msg = err?.message || '';
        if (msg.includes('already loaded') || msg.includes('already installed')) {
          continue;
        }
        vecErr = err instanceof Error ? err : new Error(String(err));
        break;
      }
    }
    if (vecErr) {
      console.error(
        `codegraph[${repoId}]: VECTOR extension failed to load on pool — ` +
          `semantic search will be unavailable. Cause: ${vecErr.message}`,
      );
    } else {
...
/**
 * Return true iff the repo has at least one row in `CodeEmbedding`.
 *
 * Treats any error (missing table, parse failure, etc.) as "no embeddings"
 * so a legacy or partial schema can never force the vector extension to load.
 *
 * Runs with stdout silenced so the native module cannot leak diagnostic
 * output into the MCP stdio stream.
 */
async function probeHasEmbeddings(conn: lbug.Connection): Promise<boolean> {
  silenceStdout();
  try {
    const queryResult = await conn.query(
      'MATCH (e:CodeEmbedding) RETURN count(e) AS n',
    );
 ...
 * Retries on lock errors (e.g., when `codegraph analyze` is running).
 *
...
async function doInitLbug(repoId: string, dbPath: string): Promise<void> {
  ...
  try {
    ...
    throw new Error(`LadybugDB not found at ${dbPath}. Run: codegraph analyze`);
  }
  ...
  if (!shared) {
    ...
    // avoids lock conflicts when `codegraph analyze` is writing.
  ...
  //
  // Why every connection (not just available[0]): LadybugDB tracks loaded
  // extensions per-connection. SHOW_INDEXES() on a connection that hadn't
  // loaded FTS reports `extension_loaded: false` even when other connections
  // on the same Database had loaded it — and queries routed to those
  // connections then return [] silently. Loading once per pool was the
  // root cause of "search returns empty for every repo" with no log line.
  //
  // Done BEFORE pool registration so no concurrent checkout can grab
  // a half-loaded connection.
  await loadExtensionsOnPool(repoId, available, shared);
...
export async function initLbugWithDb(
  ...
  // Load FTS + VECTOR extensions on EVERY connection — see comment in
  // doInitLbug for why we can't load once per Database.
  await loadExtensionsOnPool(repoId, available, shared);
