...
const _rawDims = parseInt(
  process.env.CODEGRAPH_EMBEDDING_DIMS ?? process.env.GITNEXUS_EMBEDDING_DIMS ?? '384',
  10,
);
if (Number.isNaN(_rawDims) || _rawDims <= 0) {
  const raw = process.env.CODEGRAPH_EMBEDDING_DIMS ?? process.env.GITNEXUS_EMBEDDING_DIMS;
  throw new Error(`CODEGRAPH_EMBEDDING_DIMS must be a positive integer, got "${raw}"`);
}
