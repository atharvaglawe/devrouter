...
// NOTE: Negation patterns in .codegraphignore (e.g. `!vendor/`) cannot override
...
/**
 * Load .gitignore, .codegraphignore (and legacy .gitnexusignore) rules
 * from the repo root. Returns an `ignore` instance with all patterns,
 * or null if no files found.
 */
export interface IgnoreOptions {
  /** Skip .gitignore parsing, only read .codegraphignore / .gitnexusignore. Defaults to CODEGRAPH_NO_GITIGNORE / GITNEXUS_NO_GITIGNORE env var. */
  noGitignore?: boolean;
}
...
export const loadIgnoreRules = async (
  ...
  const skipGitignore =
    options?.noGitignore ??
    (!!process.env.CODEGRAPH_NO_GITIGNORE || !!process.env.GITNEXUS_NO_GITIGNORE);
  // Order matters: later files override earlier negation/positive patterns
  // per the `ignore` package's semantics. We list .codegraphignore last so
  // it wins on conflicts with the legacy .gitnexusignore.
  const filenames = skipGitignore
    ? ['.gitnexusignore', '.codegraphignore']
    : ['.gitignore', '.gitnexusignore', '.codegraphignore'];
 ...
 * - .gitignore / .codegraphignore / .gitnexusignore patterns (via `ignore` package)
...
export const createIgnoreFilter = async (repoPath: string, options?: IgnoreOptions) => {
  ...
  return {
    ignored(p: Path): boolean {
      ...
      if (!rel) return false;
      // Check .gitignore / .codegraphignore / .gitnexusignore patterns
    ...
    childrenIgnored(p: Path): boolean {
      ...
      // Check against .gitignore / .codegraphignore / .gitnexusignore patterns.
